...
Apple and Google are both large companies. When issues crop up with their technology, it can sometimes take years to be addressed. For individual developers this can be unbelievably frustrating.

Even though it is highly encouraged for developers to build mobile apps natively, there are lots of apps and frameworks that require the use of displaying web based components. 
...
There have recently been a series of [issues](https://www.theverge.com/2021/3/22/22345696/google-android-apps-crashing-fix-system-webview) that have cropped up on Android with the WebView that has broken thousands of applications. Google addressed this [issue](), but there are a host of issues that Google has not addressed.

The WebView gives app developers a number of features that can be overriden using an interface called the WebViewClient. The WebViewClient includes many different interfaces that a developer can use to override the default behavoir of the WebView. By default on the Android WebView, a user tapping on any link will get kicked out of the current app into Chrome. You can override this by implementing the 'shouldOverrideUrlLoading' method, and returning false.

# Overriding requests

The WebViewClient also gives developers a way of overriding incoming requests, but unfortunately this feature has some use cases. To override an incoming request, you can use the 'shouldInterceptRequest' method on your WebViewClient as seen below.
...
This is an improvement over the previous implementation which only included the WebView and the String representation of the incoming URL.
...
I am guessing the idea for this method was if there was a request for a specific resource like an image or stylesheet which could be cached locally on the device, and retrieved for faster access using this method. While that is a good use case, it only works for certain types of HTTP requests, like GETS, DELETES. The 'shouldInterceptRequest' cannot really be used for POSTs and PUTs because these include a HTML body. If you are making a form POST with credentials, this method cannot be used because it does not include the body.

There are workarounds that can be used where you can override the document 'submit' event listener and the XHR object in the DOM of the web content, but this can violate the security policy of some web applications. Ultimately what we need is for Google to add the body to the 'WebResourceRequest' object. Here is what the interface for the 'WebResourceRequest' includes;
