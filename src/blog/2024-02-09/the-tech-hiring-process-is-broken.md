...
Most of the assessments I did had a timed test where the candidate was supposed to transform some sort of data structure to another structure, and modify the code so it would pass a series of unit tests. The first flaw I found with these tests is that they have the code in a sudo IDE that is in a browser window. I typically use real IDEs when I am writing code so I can use break points. One of the things these assessments do is record your keystrokes so they can detect if you are cheating, i.e. going to Google or ChatGPT to lookup answers. One of the other problems is that these are not a real world tests. The data structures I typically use are nothing like the ones I used in these assessments.
